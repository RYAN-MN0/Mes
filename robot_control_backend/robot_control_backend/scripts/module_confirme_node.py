...
class ModuleConfirm:
    def __init__(self):
        ...
        # ================= 发布者 =================
        self.pub_cmd = rospy.Publisher("/arm/cmd_vel", IntCmd, queue_size=10)
        self.pub_success = rospy.Publisher("/control/module_confirm_success", IntCmd, queue_size=10)

        # ================= 订阅者 =================
        rospy.Subscriber("/control/module_cmd", IntCmd, self.cb_upper_cmd)
        
        # ==========================
        # ✅ 修复问题2：消息类型改为 IntCmd
        # ==========================
        rospy.Subscriber("/hardware/module_cmd", IntCmd, self.cb_hardware_feedback)
